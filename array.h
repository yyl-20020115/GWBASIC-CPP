...
inline conf_array<char>::conf_array(char const* str)
	: low(1), high(strlen(str)), buf((char*)str) {
}

inline conf_array<char>::conf_array(char ch)
	: low(1), high(1), buf(&ch) {
}
...
inline void val(string const& str, integer& val, word& c)
{
	int x, n;
	if (sscanf(str.body, "%d%n", &x, &n) == 1) {
		val = x;
...
inline void val(string const& str, real& val, word& c)
{
	double x;
	int    n;
	if (sscanf(str.body, "%lf%n", &x, &n) == 1) {
