...
size_t pio_input_string(text_descriptor* td, char* dst, size_t len, int padding)
{
	size_t count = 0;
	...
	while (*fmt != '\0' && td->desc.error == IO_OK) {
		if (*fmt++ == '%') {
			...
			case 's':
			{
				...
				char* ptr = va_arg(ap, char*);
				pio_input_string(td, ptr, high - low + 1,false);
			}
			continue;
			}
		}
